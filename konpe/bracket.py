...
"""
class Bracket:
    def test_bracket_region(self, division):
        bracket = self.pairwise(division)
        for i in bracket:
            if i[0]['Region'] == i[1]['Region']:
                return False
        return True

    def test_bracket_dojo(self, division):
    ...
    def bracket_layout(self, division):
    ...
    def bracket(self, division):
        ...
        if len(division) < 7:
            ...
            for i in range(1024): #Base2 number as a joke
                random.shuffle(division)
                if self.test_bracket_region(division):
                    good_division = True
                    break
...
if __name__ == '__main__':
    ...
    B = Bracket()
    division = B.bracket(division)
    for i in B.bracket_layout(division):
        print i
